...
struct _array {
	...
	T pop(){
        ...
        app_zero(buffer,sizeof(T)*m_size);
...
struct _utility{
    ...
    static _string_array stringsplit(const _string& string, char split = ' ',bool all_new_lines = true  ){
        _string buffer;
        _string_array result;
        for(uint32_t i=0; i<string.m_count;i++){
            if(string[i] == split ){
				if( all_new_lines || (buffer.m_count>0) ){
	...
	static void string_insert(const char * in,_string * string,uint32_t start,uint32_t end){
		...
		app_zero(string_buffer,all_length);
...
#define app_shiftdown   0x100
#define app_controldown 0x200

#define app_paused      0x400

#define app_coursor_on  0x800
...
struct object_vertex {
    object_vertex(){}
    object_vertex(const object_vertex& v){ copy(v); }
    void operator = (const object_vertex& v){ copy(v); }
    void copy(const object_vertex& v){
        m_vertex = v.m_vertex;
        m_normal = v.m_normal;
        m_uv = v.m_uv;
    }
